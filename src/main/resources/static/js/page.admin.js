...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('.btn-filter')?.forEach((btn) => {
    btn.addEventListener('click', (e) => {
      ...
      document.querySelectorAll('.btn-filter').forEach((filterBtn) => {
...
async function handleUpdateUser(event) {
  ...
  if (data && data.status) {
    // Errore 4xx (Validazione, Email/Username già in uso)
...
async function loadAdminData(filter = 'all', pageIndex = 0) {
  console.log(
    `Caricamento dati Admin, filtro: ${filter}, pagina: ${pageIndex}`
  );
  if (!isAdmin()) return;
...
function renderUsersTable(usersToRender, tableBody, filter) {
  if (usersToRender.length > 0) {
    tableBody.innerHTML = usersToRender
      .map(
        (user) => `
             <tr>
                 <td>${user.id}</td>
