...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document
    .getElementById('btn-show-create-sheet-modal')
    ?.addEventListener('click', () => createSheetModal?.show());
  document
    .getElementById('btn-show-join-campaign-modal')
    ?.addEventListener('click', () => joinCampaignModal?.show());

  document
    .getElementById('createSheetForm')
    ?.addEventListener('submit', handleCreateSheet);
  document
    .getElementById('joinCampaignForm')
    ?.addEventListener('submit', handleJoinCampaign);
...
async function handleCreateSheet(event) {
  ...
  const data = await apiCall('/api/player/sheets', 'POST', {
    name,
    primaryClass,
    race,
  });
...
async function handleJoinCampaign(event) {
  ...
  const characterSheetId = document.getElementById(
    'join-campaign-sheet-id'
  )?.value;
...
async function loadPlayerData() {
        ...
        .map(
          (sheet) => `
                <div class="card">
                    ...
                    <p>ID: ${sheet.id} · ${sheet.primaryClass || '?'} Lvl ${
            sheet.primaryLevel || '?'
          } · ${sheet.race || '?'}</p>
                    
                    <a href="edit-sheet.html?id=${
                      sheet.id
  ...
  if (joinedCampaigns && campaignsList) {
    if (joinedCampaigns.length > 0) {
      campaignsList.innerHTML = joinedCampaigns
        .map(
          (jc) => `
                <div class="card">
                    <h3>${jc.campaignTitle || 'Campagna sconosciuta'}</h3>
                    <p>ID Camp: ${jc.campaignId}</p>
                    <p>Usando: ${jc.characterUsed?.name || '?'} (ID: ${
            jc.characterUsed?.id || '?'
          })</p>
                    <a href="player-campaign-detail.html?id=${
                      jc.campaignId
                    }" class="btn-primary">Entra</a>
                </div>
            `
        )
        .join('');
      ...
      campaignsList.innerHTML =
        '<p>Non fai ancora parte di nessuna campagna.</p>';
    }
  ...
  const orphanedList = document.getElementById(
    'player-orphaned-campaigns-list'
  );
  ...
  if (orphanedCampaigns && orphanedList && orphanedSection) {
    if (orphanedCampaigns.length > 0) {
      orphanedList.innerHTML = orphanedCampaigns
        .map(
          (oc) => `
                 <div class="card">
                     <h3>${
                       oc.campaignTitle || 'Campagna sconosciuta'
                     } (Orfana!)</h3>
                     <p>ID Camp: ${oc.campaignId}</p>
                     <p>Invita un nuovo Master con:</p>
                     <span class="code">${oc.inviteMastersCode}</span>
                     <small>Scadenza: ${new Date(
                       oc.deletionDeadline
  ...
  if (proposals && proposalsList && proposalsSection) {
    if (proposals.length > 0) {
        ...
        .map(
          (p) => `
                <div class="card proposal-card">
                    ...
                    <p>Data Proposta: ${new Date(
                      p.proposedDate
                    ).toLocaleString()}</p>
                    <p>Scadenza Voto: ${new Date(
                      p.expiresOn
                    ).toLocaleString()}</p>
                    ${
                      p.hasVoted
      ...
      proposalsList.innerHTML =
        '<p>Nessuna proposta di sessione attiva al momento.</p>';
      proposalsSection.style.display = 'block';
    }
