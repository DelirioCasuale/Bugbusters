...
export async function apiCall(
  ...
  try {
    ...
    if (!contentType || !contentType.includes('application/json')) {
      ...
      console.error(
        `--- API Error ${response.status} (Not JSON) ---`,
        textResponse || '<empty response>'
      );
      if (showAlerts) {
        alert(
          `Errore ${response.status}: Risposta non valida dal server. Controlla la console.`
        );
      }
      ...
      if (showAlerts) {
        alert(
          `Errore ${response.status}: ${
            data.message || 'Errore sconosciuto dal server'
          }`
        );
      }
      if (response.status === 403 || response.status === 401) {
    ...
    if (showAlerts) {
      alert(
        `Errore di rete o risposta non valida: ${error.message}. Controlla la console.`
      );
    }
    return null;
  }
