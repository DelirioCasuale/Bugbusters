...
export class Modal {
  ...
  isVisible() {
    return this.modal ? this.modal.classList.contains('show') : false;
  }
  showError(message) {
    if (this.errorDiv) {
...
export function updateGeneralUI() {
  ...
  if (isAuth && user) {
    ...
    const currentPage =
      window.location.pathname.split('/').pop() || 'landing.html';

    // 1. Crea l'avatar
    const avatarUrl = user.profileImageUrl
      ? user.profileImageUrl
      : 'images/dice.png'; // Fallback
    ...
    // 2.1. Aggiunge sempre il link Home per tornare alla landing
    if (currentPage !== 'landing.html') {
      navHTML += `<a href="landing.html">Home</a>`;
    }

    // 3. Aggiunge gli altri link (logica invariata)
    if (isAdmin()) {
      if (currentPage !== 'admin.html')
        navHTML += `<a href="admin.html">Dashboard Admin</a>`;
    } else {
...
/**
 * Inizializza la navigazione del logo per agire come bottone Home
 */
export function initLogoNavigation() {
  const logoFlex = document.querySelector('.logo-flex');
  if (!logoFlex) return;

  logoFlex.addEventListener('click', (e) => {
    e.preventDefault();
    // Logo sempre va alla landing page (Home)
